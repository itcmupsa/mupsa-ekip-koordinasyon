...
begin
  ...
  if exists (select 1 from public.events where id = target_event_id and deleted_at is null) then raise exception 'Etkinlik kalıcı olarak silinmeden önce pasifleştirilmelidir.'; end if;
...
begin
  ...
  if exists (select 1 from public.tasks where id = target_task_id and deleted_at is null) then raise exception 'Görev kalıcı olarak silinmeden önce pasifleştirilmelidir.'; end if;
...
begin
  ...
  if exists (select 1 from public.awareness_posts where id = target_awareness_post_id and deleted_at is null) then raise exception 'Farkındalık kaydı kalıcı olarak silinmeden önce pasifleştirilmelidir.'; end if;
...
begin
  ...
  if exists (select 1 from public.calendar_entries where id = target_calendar_entry_id and deleted_at is null) then raise exception 'Takvim kaydı kalıcı olarak silinmeden önce pasifleştirilmelidir.'; end if;
