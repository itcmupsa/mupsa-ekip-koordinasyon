-- Tema seçimi sürümü için, canlı frontend doğrulandıktan sonra elle başlatılan
-- tek seferlik PWA duyurusu. Migration hiçbir bildirim kuyruğu oluşturmaz.

create table public.release_announcements (
  id uuid primary key default gen_random_uuid(),
  release_key text not null unique check (char_length(trim(release_key)) between 1 and 120),
  ...
  target_url text not null check (target_url = '/app/ayarlar?section=appearance'),
...
revoke all on table public.release_announcements from authenticated;

-- Mevcut MUPİ markalama kuralını değiştirmeden yeniden tanımlar.
...
drop function if exists public.queue_theme_release_announcement();

create function public.queue_theme_release_announcement(target_period_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  -- Eşzamanlı iki servis çağrısından yalnızca biri kuyruk oluşturur.
