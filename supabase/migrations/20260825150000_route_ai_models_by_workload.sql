-- Günlük/yüksek hacimli işleri Flash-Lite'a, seyrek ve derin analizleri Flash'a yönlendirir.

update public.ai_feature_settings settings
set flash_model = 'gemini-3.7-flash',
    flash_lite_model = 'gemini-3.5-flash-lite',
    daily_flash_request_cap = 6,
    daily_flash_lite_request_cap = 100,
    policy_version = '2026-08-workload-model-routing-v4',
    updated_at = now()
...
comment on function public.reserve_ai_quota(uuid, uuid, text, text) is
  'Gemini isteğinden önce iş yüküne uygun model ve uygulama kotasını atomik olarak doğrular.';

-- Önceki Flash modeliyle üretilmiş günlük özetler saklanır ancak yeni Lite politikası altında tekrar kullanılmaz.
