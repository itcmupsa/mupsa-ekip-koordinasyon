...
- Takvim frontend ve veritabanı hazırlığı tamamlandı: mevcut etkinlik/farkındalık tarihleri tekrar kayıt oluşturulmadan takvimde gösteriliyor; manuel kayıtlar için `calendar_entries` tablosu, RLS, soft-delete ve audit eklendi. Görev son tarihleri yalnızca primary görev sahibine, `auth.uid()` kullanan `get_my_calendar_task_deadlines()` RPC'si üzerinden gösteriliyor. `Calendar.tsx`, `/app/takvim` route'u ve ana sayfa bağlantısı eklendi. `20260810050000_add_calendar_entries.sql` uzak Supabase'e uygulandı ve local/remote migration eşleşti. Lint, build ve `git diff --check` başarılı; canlı takvim kullanıcı testleri bekliyor.
