...
- İkinci Google anahtarı `GEMINI_API_KEY_SECONDARY` adıyla yalnızca canlı Supabase secret alanına eklendi. Birincil anahtar kimlik doğrulama, kota veya geçici servis hatası verirse sunucu ikincil anahtarı bir kez dener; iki anahtar da kullanılamazsa API'siz doğrulanmış özet çalışmaya devam eder. Anahtarlar frontend'e, GitHub'a veya uygulama loglarına yazılmaz.
...
- `ai-orchestrator` Edge Function aynı projeye deploy edildi. Ana sayfa özeti 24 saat önbellek, 6 saatlik zorunlu yenileme aralığı, eski doğrulanmış özete geri dönüş ve sunucu tarafı rota çözümleme kurallarıyla hazırlanır.
...
- Günlük özet v3 kuralı: etkinlik eksikleri yalnızca otomatik `preparation_start_date` geldikten sonra değerlendirilir; 40 günlük hazırlık dönemi başlamamış etkinlikler eksik mekân/SKS nedeniyle öne çıkarılmaz. Farkındalık eksikleri de kendi hazırlık başlangıcından önce önerilmez. Çıktı en fazla üç gerçek öncelikten oluşur, kompakt liste olarak gösterilir ve normal kullanımda 24 saatlik önbellekten sunulur.
- Günlük ortak özet Gemini 3.7 Flash ile dönem başına günde yalnızca bir kez üretilir ve bütün Süper Yöneticiler aynı doğrulanmış 24 saatlik çıktıyı kullanır. 3.7 Flash uygulama havuzu derin analizlerle birlikte günde 20, yüksek hacimli Flash-Lite işlemleri günde 100 rezervasyonla sınırlandırılır; sağlayıcı kotası daha önce dolarsa ikincil proje anahtarı denenir ve sonrasında API'siz güvenli yedek çalışır.
- Uygulama veya Google günlük kotası dolduğunda ana sayfa artık hata göstermez. Aynı yetki, 40 günlük hazırlık ve öncelik kurallarıyla doğrulanmış kayıtlardan API kullanmadan en fazla üç maddelik yedek özet üretir; canlı testte yalnızca yaklaşan `MUPSA 2. Online Toplantı` kaydı gösterildi.
- Model yönlendirme v5 kararı: dönem başına günde tek ortak günlük özet, takvim dahil derin dönem analizi, haftalık yönetim analizi ve çok kaynaklı kurumsal hafıza sentezi `gemini-3.7-flash`; normal sohbet, kısa taslak, manuel takvim sınıflandırması ve önemli gün önerileri `gemini-3.5-flash-lite` kullanır. 3.7 normal sohbet mesajlarında kullanılmaz.
