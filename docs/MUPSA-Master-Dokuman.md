...
**Son güncelleme:** Faz 1 / Adım 1 tamamlandı ve gerçek Supabase Free projesinde uygulandı. Faz 1 / Adım 2 (bildirim motoru) kademeli olarak ilerliyor: görev ataması ve SKS durum değişikliği, uygulama içi ve e-posta teslimat kuyruğuna bildirim üretir. SKS değişikliği, karar gereği etkinliğin dönemindeki tüm aktif ekibe gider. SKS onayı veya kaynak görevin tamamlanmasıyla, tüm yapılandırılmış koşulları sağlanan taslak görevler otomatik Aktif olur ve yalnızca o görevdeki atanmış kişilere bildirim gider; tarih bazlı bağımlılıklar sonraki pg_cron adımında aktive edilecektir. Etkinlik tarihindeki değişiklik, etkinlik sahibi, etkinlik/süreç üyeleri ve o etkinlikte görevi olan aktif kişilere gider; görev tarihleri otomatik kaydırılmaz. Push altyapısı için, kullanıcının birden fazla cihaz/tarayıcı aboneliğini ayrı tutan `push_subscriptions` tablosu eklendi; teknik abonelik anahtarları audit geçmişine yazılmaz. Migration'lar; kullanıcı profilleri ve roller, dönemler, etkinlikler, görevler, çok kişili atamalar, yapılandırılmış bağımlılıklar, bildirim kuyruğu, dönem kilidi, geri alınabilir silme, audit geçmişi ve Row Level Security kurallarıyla gerçek veritabanında çalıştırıldı. Dönem üyeliğindeki rol, süper yönetici, aktif/pasif ve üyelik değişiklikleri de audit geçmişine girer. Görev atama, push, SKS, bağımlılık aktivasyonu ve tarih değişikliği testleri gerçek veritabanı işlemi içinde çalıştırılıp geri alındı; test kaydı bırakılmadı.

**Şu an neredeyiz:** Faz 1 / Adım 1 tamamlandı. Faz 1 / Adım 2'de “görev atandı”, “SKS durumu değişti”, “bağımlı görev aktifleşti” ve “etkinlik tarihi değişti” tetikleyicileri, push abonelik altyapısı ve test senaryoları eklendi; gerçek Supabase projesinde uygulama ve temel işlev testleri başarıyla tamamlandı. İlk migration'lar için Supabase CLI bağlantısı ve uzak migration geçmişi baseline senkronizasyonu da tamamlandı. Sıradaki bildirim parçası pg_cron ile zamanlanmış kontroller.
