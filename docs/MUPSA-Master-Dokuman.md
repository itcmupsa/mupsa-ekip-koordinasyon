...
**Şu an neredeyiz:** Faz 1 / Adım 1 tamamlandı. Faz 1 / Adım 2'de “görev atandı” tetikleyicisi, push abonelik altyapısı ve test senaryoları eklendi; gerçek Supabase projesinde uygulama ve temel işlev testleri başarıyla tamamlandı. Sıradaki bildirim parçası SKS durum değişikliği; ardından bağımlılık aktivasyonu ve zamanlanmış kontroller ayrı ayrı eklenecek. **Önemli teknik not:** İlk dört dosya SQL Editor ile elle uygulandığı için yeni migration yazmadan önce proje Supabase CLI ile repoya bağlanıp migration geçmişi için güvenli baseline oluşturulmalıdır.
