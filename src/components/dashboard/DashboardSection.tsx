...
interface DashboardSectionProps { title: string; children: ReactNode; actionLabel?: string; actionTo?: string; countLabel?: string; tone?: DashboardSectionTone; className?: string }
...
export default function DashboardSection({ title, children, actionLabel, actionTo, countLabel, tone = 'default', className = '' }: DashboardSectionProps) {
  return (
    <section className={['rounded-xl border border-canvas-border bg-canvas-surface p-4 shadow-card sm:p-5', className].filter(Boolean).join(' ')}>
      <div className="mb-3 flex flex-wrap items-start justify-between gap-3">
        ...
        {actionLabel && actionTo ? <Link to={actionTo} className="flex shrink-0 items-center gap-1 rounded-md px-2 py-1 text-sm font-medium text-brand-dark hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2"><span>{actionLabel}</span><ChevronIcon /></Link> : null}
      </div>
      {children}
    </section>
  )
