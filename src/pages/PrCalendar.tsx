...
export default function PrCalendar({ session }: { session: Session }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const prEntries = (entryResult.data ?? []).map((row) => ({ id: row.id, periodId: row.period_id, title: row.title, entryKind: row.entry_kind, scheduledDate: row.scheduled_date, scheduledTime: row.scheduled_time, color: row.color, status: row.status, channels: row.channels ?? (row.channel ? [row.channel] : []), format: row.format, notes: row.notes, responsibleId: row.responsible_id, eventId: row.event_id, awarenessPostId: row.awareness_post_id, taskId: row.task_id, relatedPrEntryId: row.related_pr_entry_id, referenceLabel: row.reference_label, referenceUrl: row.reference_url, deletedAt: row.deleted_at } as PrEntry))
  ...
  async function saveEntry() {
    ...
    const payload = { period_id: periodId, title: draft.title.trim(), entry_kind: draft.entryKind, scheduled_date: draft.scheduledDate, scheduled_time: draft.scheduledTime || null, color: draft.color, status: draft.status, channels: draft.channels, channel: draft.channels[0] ?? null, format: draft.format.trim() || null, notes: draft.notes.trim() || null, responsible_id: draft.responsibleId || null, event_id: draft.eventId || null, awareness_post_id: draft.awarenessPostId || null, task_id: draft.taskId || null, related_pr_entry_id: draft.relatedPrEntryId || null, reference_label: draft.referenceUrl.trim() ? draft.referenceLabel.trim() || null : null, reference_url: draft.referenceUrl.trim() || null }
