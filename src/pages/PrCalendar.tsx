import ChannelPicker from '../components/calendar/PrChannelPicker'
import { useCallback, useEffect, useMemo, useRef, useState, type ReactNode } from 'react'
import type { Session } from '@supabase/supabase-js'
import { Link, useSearchParams } from 'react-router-dom'
import CalendarTabs from '../components/calendar/CalendarTabs'
import PrAssigneeSelect from '../components/calendar/PrAssigneeSelect'
import ManualCalendarEntryDialog, { type ManualCalendarRecord } from '../components/calendar/ManualCalendarEntryDialog'
import PrReferenceLinksField from '../components/calendar/PrReferenceLinksField'
...
interface PrEntry {
  ...
  assignees: PrEntryAssignee[]
  eventId: string | null
  awarenessPostId: string | null
  taskId: string | null
  relatedPrEntryId: string | null
  referenceLinks: PrReferenceLink[]
...
export default function PrCalendar({ session }: { session: Session }) {
  ...
  const filtered = useMemo(() => entries.filter((item) =>
    (showDeleted || !item.deletedAt) && (statusFilter === 'all' || item.status === statusFilter) && (kindFilter === 'all' || item.entryKind === kindFilter) && (responsibleFilter === 'all' || item.assignees.some(a => a.profileId === responsibleFilter)) && (!query.trim() || `${item.title} ${item.channels.join(' ')} ${item.format ?? ''}`.toLocaleLowerCase('tr').includes(query.toLocaleLowerCase('tr')))),
  ...
  async function saveEntry() {
    if (!periodId || !profileId || (!canManage && !selected) || saving) return
    ...
    const referenceLinksError = validatePrReferenceLinks(draft.referenceLinks)
    if (referenceLinksError) { setFormError(referenceLinksError); return }
    if (draft.eventId && draft.awarenessPostId) { setFormError('Etkinlik ve farkındalık kaydı aynı anda seçilemez.'); return }
    setSaving(true); setFormError(null)

    const referenceLinks = normalizePrReferenceLinks(draft.referenceLinks)
    if (canManage) {
      const firstReferenceLink = referenceLinks[0]
      const legacyResponsibleId = computeLegacyResponsibleId(selected?.responsibleId, draft.manualAssigneeIds)

      const { error } = await supabase.rpc('upsert_pr_entry_with_manual_assignees', {
        p_id: selected?.id || null,
        p_period_id: periodId,
        p_title: draft.title.trim(),
        p_entry_kind: draft.entryKind,
        p_scheduled_date: draft.scheduledDate,
        p_scheduled_time: draft.scheduledTime || null,
        p_color: draft.color,
        p_status: draft.status,
        p_channels: draft.channels,
        p_channel: draft.channels[0] ?? null,
        p_format: draft.format.trim() || null,
        p_notes: draft.notes.trim() || null,
        p_responsible_id: legacyResponsibleId,
        p_event_id: draft.eventId || null,
        p_awareness_post_id: draft.awarenessPostId || null,
        p_task_id: draft.taskId || null,
        p_related_pr_entry_id: draft.relatedPrEntryId || null,
        p_reference_links: referenceLinks,
        p_reference_label: firstReferenceLink?.label ?? null,
        p_reference_url: firstReferenceLink?.url ?? null,
        p_manual_assignees: draft.manualAssigneeIds
      })
      if (error) { setFormError(error.message); setSaving(false); return }
    } else {
      const payload = buildNonManagerPayload(draft, referenceLinks)
      const { error } = await supabase.from('pr_calendar_entries').update(payload).eq('id', selected!.id)
      if (error) { setFormError(error.message); setSaving(false); return }
    }
  ...
  useEffect(() => {
    ...
    if (!day || Number.isNaN(parseDateOnly(day).getTime())) return
        ...
        <div className="flex flex-col gap-3 border-b border-canvas-border pb-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex rounded-xl bg-canvas p-1" aria-label="Takvim görünümü">
              <button type="button" aria-pressed={view === 'month'} onClick={() => { if (view !== 'month') { setView('month'); setMonth(weekStart.slice(0, 8) + '01') } }} className={`min-h-11 rounded-lg px-4 text-sm font-medium transition-colors ${view === 'month' ? 'bg-brand-dark text-white shadow-sm' : 'text-ink-soft hover:text-ink'}`}>Ay</button>
              <button type="button" aria-pressed={view === 'week'} onClick={() => { if (view !== 'week') { const firstWeek = mondayOfWeek(month); setView('week'); setWeekStart(firstWeek); setSelectedDay(firstWeek) } }} className={`min-h-11 rounded-lg px-4 text-sm font-medium transition-colors ${view === 'week' ? 'bg-brand-dark text-white shadow-sm' : 'text-ink-soft hover:text-ink'}`}>Hafta</button>
            </div>
          </div>
        </div>
...
function SelectedEntryPanel({ entry, memberName, events, awareness, tasks, entries, canManage, profileId, onOpen }: { entry: PrEntry; memberName: (id: string | null) => string; events: Source[]; awareness: Source[]; tasks: Source[]; entries: PrEntry[]; canManage: boolean; profileId: string | null; onOpen: (trigger: HTMLElement) => void }) {
  ...
  const reference = entry.referenceLinks.map((link) => link.label).join(', ') || 'Yok'
  ...
  return <section aria-labelledby="selected-pr-entry-title" className="mt-3 rounded-xl border border-canvas-border bg-white p-4 shadow-card sm:p-5">
    ...
    <dl className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <DetailItem label="Sorumlu" value={entry.assignees.length > 0 ? entry.assignees.map(a => memberName(a.profileId)).join(', ') : 'Atanmamış'}><Icon><path d="M20 21a8 8 0 0 0-16 0M12 13a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" /></Icon></DetailItem>
      ...
      <DetailItem label="Bağlantılar" value={reference}><Icon><path d="M10 13a5 5 0 0 0 7.1.1l2-2a5 5 0 0 0-7.1-7.1M14 11a5 5 0 0 0-7.1-.1l-2 2A5 5 0 0 0 12 20" /></Icon></DetailItem>
    </dl>
  </section>
}
...
function Select({ label, value, options, onChange, disabled }: { label: string; value: string; options: Array<{ value: string; label: string } | Source>; onChange: (value: string) => void; disabled?: boolean }) { return <label className="block text-sm font-medium">{label}<select disabled={disabled} value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 min-h-[44px] w-full rounded-lg border border-canvas-border px-3 font-normal">{options.map((option) => <option key={'id' in option ? option.id : option.value} value={'id' in option ? option.id : option.value}>{'title' in option ? option.title : option.label}</option>)}</select></label> }
function Text({ label, value, onChange, maxLength = 120 }: { label: string; value: string; maxLength?: number; onChange: (value: string) => void }) { return <label className="block text-sm font-medium">{label}<input maxLength={maxLength} value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 min-h-[44px] w-full rounded-lg border border-canvas-border px-3 font-normal" /></label> }
function EntryDetails({ entry, memberName, events, awareness, tasks, entries }: { entry: PrEntry; memberName: (id: string | null) => string; events: Source[]; awareness: Source[]; tasks: Source[]; entries: PrEntry[] }) { const source = (list: Source[], id: string | null) => list.find((item) => item.id === id)?.title; return <div className="space-y-4 p-5"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-canvas px-2.5 py-1 text-xs font-medium">{formatOptionalTime(entry.scheduledTime)} · {fullDate(entry.scheduledDate)}</span><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${statusClass(entry.status)}`}>{labelFor(PR_ENTRY_STATUSES, entry.status)}</span></div><p className="text-sm text-ink-soft">Sorumlu: <span className="font-medium text-ink">{entry.assignees.length > 0 ? entry.assignees.map(a => memberName(a.profileId)).join(', ') : 'Atanmamış'}</span></p>{entry.channels.length > 0 || entry.format ? <p className="text-sm text-ink-soft">{[...entry.channels, entry.format].filter(Boolean).join(' · ')}</p> : null}{entry.notes ? <p className="whitespace-pre-wrap rounded-lg bg-canvas p-3 text-sm text-ink">{entry.notes}</p> : null}<div className="space-y-2 text-sm">{entry.eventId ? <Link to={`/app/etkinlikler/${entry.eventId}`} className="block text-brand-dark underline">Etkinlik: {source(events, entry.eventId) ?? 'Kaydı aç'}</Link> : null}{entry.awarenessPostId ? <Link to={`/app/farkindalik?record=${entry.awarenessPostId}`} className="block text-brand-dark underline">Farkındalık: {source(awareness, entry.awarenessPostId) ?? 'Kaydı aç'}</Link> : null}{entry.taskId ? <Link to={`/app/gorevler?task=${entry.taskId}`} className="block text-brand-dark underline">Görev: {source(tasks, entry.taskId) ?? 'Kaydı aç'}</Link> : null}{entry.relatedPrEntryId ? <Link to={`/app/takvimler/pr?entry=${entry.relatedPrEntryId}`} className="block text-brand-dark underline">Bağlı PR kaydı: {entries.find((item) => item.id === entry.relatedPrEntryId)?.title ?? 'Kayıt'}</Link> : null}{entry.referenceLinks.map((link) => isSafeExternalUrl(link.url) ? <a key={link.id} href={link.url} target="_blank" rel="noreferrer" className="block break-words text-brand-dark underline">{link.label}</a> : null)}</div></div> }
